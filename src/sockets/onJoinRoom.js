  ...
  Socket.client.on(EVENTS.LISTEN.JOIN_ROOM, data => {
    ...
    if (room) {
      if (room.status === GAME_STATUS.WAITING) {
        const isNameValid = !room.players.filter(
        ...
        if (isNameValid) {
          ...
          Socket.socket
            .to(data.roomId)
            .emit(EVENTS.EMIT.GAME_STATUS_UPDATE, { ...room });
        } else
          Socket.client.emit(EVENTS.EMIT.JOIN_ROOM_RESPONSE, {
            error: 'Toks vardas jau yra'
          });
      } else
        Socket.client.emit(EVENTS.JOIN_ROOM_RESPONSE, {
          error: 'Kambarys užimtas'
        });
    } else
      Socket.client.emit(EVENTS.EMIT.JOIN_ROOM_RESPONSE, {
        error: 'Kambarys nerastas'
      });
  });
};
